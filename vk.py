    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW:
            ...
            if event.to_me:
                ...
                #     keyboard.add_line()
                #     keyboard.add_button("⏪Назад⏪")
                #     sender(id, "Дистрибуция - публикация Вашего трека/Альбома на все цифровые площадки.\nКаждый релиз отправляется на модерацию ПРОМО всех стриминговых сервисов.\nСсылка на портфолио - (позже)\n\n💸 Цена: 500 ₽.\n⏳ Сроки: от 3 до 5 дней.", keyboard)
                # if msg == "сведение и мастеринг":
                #     keyboard = VkKeyboard()
                #     keyboard.add_button("✅Подтвердить заказ✅", VkKeyboardColor.POSITIVE)
                #     keyboard.add_line()
                #     keyboard.add_button("⏪Назад⏪")
                #     sender(id, "Сведение и Мастеринг - выполнение комплекса работ по постобработке музыкального произведения.\nОчистка и подготовка звуковых дорожек, Обработка вокала, Питч-коррекция. Добавление различных эффектов и особенностей, Расширение стереобазы, работа с панорамой, Эквализация и компрессия,  и многое другое.\nВсе Ваши пожелания и требования будут учтены в процессе работы. Вам доступно Три бесплатных правки итоговой работы.\nСсылка на портфолио - (позже)\n\n💸 Цена: от 2500 ₽.\n⏳ Сроки: от 3 до 5 дней.", keyboard)
                # if msg == "создание дизайна":
                #     keyboard = VkKeyboard()
                #     keyboard.add_button("✅Подтвердить заказ✅", VkKeyboardColor.POSITIVE)
                #     keyboard.add_line()
                #     keyboard.add_button("⏪Назад⏪")
                #     sender(id, "Создание дизайна - Создание обложки для Релиза/Альбома, Ретушь и обработка фото, создание дизайна для ПРОМО.\nВсе Ваши пожелания и требования будут учтены в процессе работы. Вам доступно Три бесплатных правки итоговой работы.\nСсылка на портфолио - https://vk.com/album-221967488_295470314\n\n💸 Цена: от 1500 ₽.\n⏳ Сроки: от 3 до 5 дней.", keyboard)
                # if msg == "аренда бита":
                #     keyboard = VkKeyboard()
                #     keyboard.add_button("✅Подтвердить заказ✅", VkKeyboardColor.POSITIVE)
                #     keyboard.add_line()
                #     keyboard.add_button("⏪Назад⏪")
                #     sender(id, "Аренда бита - любая категория аренды бита, от MP3 до EXCLUSIVE. Все биты представленные в плейлистах доступны для покупки.\nПлейлисты разбиты по разным Авторам и  жанрам, выбирайте подходящий Вам звук.\n\nСсылка на плейлист 1 - (позже)\n\n💸 Цена: от 1500 ₽.\n⏳ Сроки: 1 день.", keyboard)
                # if msg == "создание трека":
                #     keyboard = VkKeyboard()
                #     keyboard.add_button("✅Подтвердить заказ✅", VkKeyboardColor.POSITIVE)
                #     keyboard.add_line()
                #     keyboard.add_button("⏪Назад⏪")
                #     sender(id, "Создание трека - услуга в полном объеме временно недоступна.\n\n💸 Цена:  по договорённости\n⏳ Сроки: по договорённости", keyboard)
